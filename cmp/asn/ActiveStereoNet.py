...
class ResNetBlock(nn.Module):
    def __init__(self, in_channel, out_channel, stride, downsample, pad, dilation):
        ...
        self.conv1 = nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=1, padding=1, dilation=dilation)
        self.downsample = downsample
        self.stride = stride
...
class ActiveStereoNet(nn.Module):
    def __init__(self, max_disp=300, bias_disp=100):
        super(ActiveStereoNet, self).__init__()
        self.disp_alpha = max_disp - bias_disp
        self.disp_beta = bias_disp
    ...
    def forward(self, left, right):
        disp = (self.disp_alpha + 1) // 8
        bias = (self.disp_beta + 1) // 8
        ...
        def calculate(refimg_feature, targetimg_feature, img, type):
            ...
            if type == 'left':
                for a in range(disp):
                    d = a + bias
                    if d > 0:
                        cost[:, :, a, :, d:] = refimg_feature[:, :, :, d:] - targetimg_feature[:, :, :, :-d]
                    else:
                        cost[:, :, a, :, :] = refimg_feature - targetimg_feature
            ...
            pred = disparityregression(disp)(pred) + bias
        ...
        return res_disparityL, Full_res_disparityL  # Full_res_disparityR, Full_res_disparityR
